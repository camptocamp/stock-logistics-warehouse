...
class StockInventory(models.Model):
    ...
    def _block_location_on_inventory(self):
        ...
        return self.filtered(
            lambda inv: (
                inv.location_id.company_id or inv.env.company
            ).block_location_on_inventory
        )

    def _affected_inventory_lock_locations(self):
        loc_ids = self.mapped("location_id").ids
        if not loc_ids:
            return self.env["stock.location"]
        return self.env["stock.location"].search([("id", "child_of", loc_ids)])
    ...
    def write(self, vals):
        ...
        track = bool(relevant and {"state", "location_id"} & set(vals))
